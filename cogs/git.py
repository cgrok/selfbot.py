...
class Git:
    ...
    async def comment(self, ctx, repo, issueid:int, *, content):
        async with ctx.session.post(f'https://api.github.com/repos/{repo}/issues/{issueid}/comments', json={"body": content}, headers={'Authorization': f'Bearer {await self.githubtoken()}'}) as resp:
